...
a = 0.24  # ALPHA: rate from latent to infection
...
for (i in 1:T){
  for (j in 1:10){  
    ...
    # initals
    S[1,j] =  da_alpha2[1,paste("S.B", j, sep = "")]
    E[1,j] =  da_alpha2[1,paste("E.B", j, sep = "")]
    D[1,j] =  da_alpha2[1,paste("D.B", j, sep = "")]
    U[1,j] =  da_alpha2[1,paste("U.B", j, sep = "")]
    Q[1,j] =  da_alpha2[1,paste("Q.B", j, sep = "")]
    R[1,j] =  da_alpha2[1,paste("R.B", j, sep = "")]
    
    # STUDENT to STUDENT in the SAME BUBBLE
    S1[1,j] =  da_alpha2[1,paste("S.B", j, sep = "")]
    E1[1,j] =  da_alpha2[1,paste("E.B", j, sep = "")]
    D1[1,j] =  da_alpha2[1,paste("D.B", j, sep = "")]
    U1[1,j] =  da_alpha2[1,paste("U.B", j, sep = "")]
    Q1[1,j] =  da_alpha2[1,paste("Q.B", j, sep = "")]
    R1[1,j] =  da_alpha2[1,paste("R.B", j, sep = "")]
    ...
    all.S = (c(1:ncol(da_alpha2)) %% 7) == 1
    all.E = (c(1:ncol(da_alpha2)) %% 7) == 2
    all.D = (c(1:ncol(da_alpha2)) %% 7) == 3
    all.U = (c(1:ncol(da_alpha2)) %% 7) == 4
    all.R = (c(1:ncol(da_alpha2)) %% 7) == 6
    ...
    S2.diff[i,j] = round(sum((da_alpha2[i,(all.S)][,-j])[,c(1:ncol(da_alpha2[i,(all.S)][,-j])-1)]))
    E2.diff[i,j] = round(sum((da_alpha2[i,(all.E)][,-j])[,c(1:ncol(da_alpha2[i,(all.E)][,-j])-1)]))
    D2.diff[i,j] = round(sum((da_alpha2[i,(all.D)][,-j])[,c(1:ncol(da_alpha2[i,(all.D)][,-j])-1)]))
    U2.diff[i,j] = round(sum((da_alpha2[i,(all.U)][,-j])[,c(1:ncol(da_alpha2[i,(all.U)][,-j])-1)]))
    R2.diff[i,j] = round(sum((da_alpha2[i,(all.R)][,-j])[,c(1:ncol(da_alpha2[i,(all.R)][,-j])-1)]))
    ...
    # Add the results of each time step i and each bubble j of each compartments to the dataframe da_alpha2
    da_alpha2[i,paste("S.B", j, sep = "")] <- S[i,j]
    da_alpha2[i,paste("E.B", j, sep = "")] <- E[i,j]
    da_alpha2[i,paste("D.B", j, sep = "")] <- D[i,j]
    da_alpha2[i,paste("U.B", j, sep = "")] <- U[i,j]
    da_alpha2[i,paste("Q.B", j, sep = "")] <- Q[i,j]
    da_alpha2[i,paste("R.B", j, sep = "")] <- R[i,j]
    
    da_alpha2[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,]) + sum(S.Stf[i,])
    da_alpha2[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,]) + sum(E.Stf[i,])
    da_alpha2[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,]) + sum(D.Stf[i,])
    da_alpha2[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,]) + sum(U.Stf[i,])
    da_alpha2[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,]) + sum(Q.Stf[i,])
    da_alpha2[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,]) + sum(R.Stf[i,])
    
    # Sum up the total dynamic of students for all bubbles
    da_alpha2$S.Student[i] <- sum(S[i,])
    da_alpha2$E.Student[i] <- sum(E[i,])
    da_alpha2$D.Student[i] <- sum(D[i,])
    da_alpha2$U.Student[i] <- sum(U[i,])
    da_alpha2$Q.Student[i] <- sum(Q[i,])
    da_alpha2$R.Student[i] <- sum(R[i,])
    da_alpha2$N.Student[i] <- 100
...
for (i in 1:T){
  ...
  da_alpha2$Staff.rp[i] <- sum(Staff.rp[i,])  
  
  # reproduction rate Rt
  da_alpha2$Rt.Student[i] <- (da_alpha2$E.Student[i+1] + da_alpha2$D.Student[i+1] + da_alpha2$U.Student[i+1] + da_alpha2$Q.Student[i+1]) / 
    (da_alpha2$E.Student[i] + da_alpha2$D.Student[i] + da_alpha2$U.Student[i] + da_alpha2$Q.Student[i])
  
  da_alpha2$Rt.Staff[i] <- (da_alpha2$E.Staff[i+1] + da_alpha2$D.Staff[i+1] + da_alpha2$U.Staff[i+1] + da_alpha2$Q.Staff[i+1]) / 
    (da_alpha2$E.Staff[i] + da_alpha2$D.Staff[i] + da_alpha2$U.Staff[i] + da_alpha2$Q.Staff[i])
}
...
# Students
X_lim <- seq(1,T,by=1)
plot(da_alpha2$S.Student~X_lim, pch=15, col="black", main = "SEDUQR Model for Students", type = "l", xlab = "Time (day)", ylab = "Number of Students", xlim = c(0,T), ylim = c(0,100))

lines(da_alpha2$S.Student, col="DeepPink", lty=1)
lines(da_alpha2$E.Student, col="darkTurquoise", lty=1)
lines(da_alpha2$D.Student, col="RosyBrown", lty=1)
lines(da_alpha2$U.Student, col="green", lty=1)
lines(da_alpha2$Q.Student, col="darkgreen", lty=1)
lines(da_alpha2$R.Student, col="darkblue", lty=1)
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da_alpha2$S.Staff~X_lim, pch=15, col="black", main = "SEDUQR Model for Staffs", type = "l", xlab = "Time (day)", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,35))

lines(da_alpha2$S.Staff, col="DeepPink", lty=1)
lines(da_alpha2$E.Staff, col="darkTurquoise", lty=1)
lines(da_alpha2$D.Staff, col="RosyBrown", lty=1)
lines(da_alpha2$U.Staff, col="green", lty=1)
lines(da_alpha2$Q.Staff, col="darkgreen", lty=1)
lines(da_alpha2$R.Staff, col="darkblue", lty=1)
