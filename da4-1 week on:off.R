# Variation in SCHOOL OPENING TIME
# (da4)
###################################################################################################################
...
A[,7,11] <- 50 #  50 staffs in total (20 of them work from home)
...
head(A)
da4 <- as.data.frame(A)
da41=da42=da43=da44=da45  # store the data for long weekend, 1 day on/off, 2 days on/off, 1 week on/off and 2 weeks on/off
...
for (i in 1:T){
  for (j in 1:10){  
    ...
    # initals
    S[1,j] =  da4[1,paste("S.B", j, sep = "")]
    E[1,j] =  da4[1,paste("E.B", j, sep = "")]
    D[1,j] =  da4[1,paste("D.B", j, sep = "")]
    U[1,j] =  da4[1,paste("U.B", j, sep = "")]
    Q[1,j] =  da4[1,paste("Q.B", j, sep = "")]
    R[1,j] =  da4[1,paste("R.B", j, sep = "")]
    
    # STUDENT to STUDENT in the SAME BUBBLE
    S1[1,j] =  da4[1,paste("S.B", j, sep = "")]
    E1[1,j] =  da4[1,paste("E.B", j, sep = "")]
    D1[1,j] =  da4[1,paste("D.B", j, sep = "")]
    U1[1,j] =  da4[1,paste("U.B", j, sep = "")]
    Q1[1,j] =  da4[1,paste("Q.B", j, sep = "")]
    R1[1,j] =  da4[1,paste("R.B", j, sep = "")]
    ...
    all.S = (c(1:ncol(da4)) %% 7) == 1
    all.E = (c(1:ncol(da4)) %% 7) == 2
    all.D = (c(1:ncol(da4)) %% 7) == 3
    all.U = (c(1:ncol(da4)) %% 7) == 4
    all.R = (c(1:ncol(da4)) %% 7) == 6
    ...
    S2.diff[i,j] = round(sum((da4[i,(all.S)][,-j])[,c(1:ncol(da4[i,(all.S)][,-j])-1)]))
    E2.diff[i,j] = round(sum((da4[i,(all.E)][,-j])[,c(1:ncol(da4[i,(all.E)][,-j])-1)]))
    D2.diff[i,j] = round(sum((da4[i,(all.D)][,-j])[,c(1:ncol(da4[i,(all.D)][,-j])-1)]))
    U2.diff[i,j] = round(sum((da4[i,(all.U)][,-j])[,c(1:ncol(da4[i,(all.U)][,-j])-1)]))
    R2.diff[i,j] = round(sum((da4[i,(all.R)][,-j])[,c(1:ncol(da4[i,(all.R)][,-j])-1)]))
    ...
    # Assume there are 50 staffs in total, initially 30 of them teach students. 
    ...
    # Add the results of each time step i and each bubble j of each compartments to the dataframe da4
    da4[i,paste("S.B", j, sep = "")] <- S[i,j]
    da4[i,paste("E.B", j, sep = "")] <- E[i,j]
    da4[i,paste("D.B", j, sep = "")] <- D[i,j]
    da4[i,paste("U.B", j, sep = "")] <- U[i,j]
    da4[i,paste("Q.B", j, sep = "")] <- Q[i,j]
    da4[i,paste("R.B", j, sep = "")] <- R[i,j]
    
    da4[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,]) 
    da4[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,]) 
    da4[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,])  
    da4[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,])  
    da4[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,])  
    da4[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,])  
    
    # Sum up the total dynamic of students for all bubbles
    da4$S.Student[i] <- sum(S[i,])
    da4$E.Student[i] <- sum(E[i,])
    da4$D.Student[i] <- sum(D[i,])
    da4$U.Student[i] <- sum(U[i,])
    da4$Q.Student[i] <- sum(Q[i,])
    da4$R.Student[i] <- sum(R[i,])
    da4$N.Student[i] <- 100
...
for (i in 1:T){
  ...
  da4$Staff.rp[i] <- sum(Staff.rp[i,])  
  
  # reproduction rate Rt
  da4$Rt.Student[i] <- (da4$E.Student[i+1] + da4$D.Student[i+1] + da4$U.Student[i+1] + da4$Q.Student[i+1]) / 
    (da4$E.Student[i] + da4$D.Student[i] + da4$U.Student[i] + da4$Q.Student[i])
  
  da4$Rt.Staff[i] <- (da4$E.Staff[i+1] + da4$D.Staff[i+1] + da4$U.Staff[i+1] + da4$Q.Staff[i+1]) / 
    (da4$E.Staff[i] + da4$D.Staff[i] + da4$U.Staff[i] + da4$Q.Staff[i])
}
...
# Students
X_lim <- seq(1,T,by=1)
plot(da4$S.Student~X_lim, pch=15, col="darkTurquoise", main = "Variation in Time, 1 week on 1 week off school - Students", type = "l", xlab = "Time", ylab = "Number of Students", xlim = c(0,T), ylim = c(0,100))

lines(da4$S.Student, col="DeepPink", lty=1)
lines(da4$E.Student, col="darkTurquoise", lty=1)
lines(da4$D.Student, col="RosyBrown", lty=1)
lines(da4$U.Student, col="green", lty=1)
lines(da4$Q.Student, col="darkgreen", lty=1)
lines(da4$R.Student, col="darkblue", lty=1)

legend(300, 95 ,c("S","E","D", "U", "Q", "R"),col=c("DeepPink","darkTurquoise","RosyBrown", "green", "darkgreen", "darkblue"),
       text.col=c("DeepPink","darkTurquoise","RosyBrown", "green", "darkgreen", "darkblue"),lty=c(1,1,1,1),
       cex=0.5)
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da4$S.Staff~X_lim, pch=15, col="darkTurquoise", main = "Variation in Time, 1 week on 1 week off school - Staffs", type = "l", xlab = "Time", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,35))

lines(da4$S.Staff, col="DeepPink", lty=1)
lines(da4$E.Staff, col="darkTurquoise", lty=1)
lines(da4$D.Staff, col="RosyBrown", lty=1)
lines(da4$U.Staff, col="green", lty=1)
lines(da4$Q.Staff, col="darkgreen", lty=1)
lines(da4$R.Staff, col="darkblue", lty=1)
