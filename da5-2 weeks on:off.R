...
o = 0.5  # THETA: rate of getting quanrantined 
...
A[1,,1] <- c(9,0,1,0,0,0,10)   # initials
...
for (i in 1:T){
  for (j in 1:10){  
    ...
    # initals
    S[1,j] =  da5[1,paste("S.B", j, sep = "")]
    E[1,j] =  da5[1,paste("E.B", j, sep = "")]
    D[1,j] =  da5[1,paste("D.B", j, sep = "")]
    U[1,j] =  da5[1,paste("U.B", j, sep = "")]
    Q[1,j] =  da5[1,paste("Q.B", j, sep = "")]
    R[1,j] =  da5[1,paste("R.B", j, sep = "")]
    
    # STUDENT to STUDENT in the SAME BUBBLE
    S1[1,j] =  da5[1,paste("S.B", j, sep = "")]
    E1[1,j] =  da5[1,paste("E.B", j, sep = "")]
    D1[1,j] =  da5[1,paste("D.B", j, sep = "")]
    U1[1,j] =  da5[1,paste("U.B", j, sep = "")]
    Q1[1,j] =  da5[1,paste("Q.B", j, sep = "")]
    R1[1,j] =  da5[1,paste("R.B", j, sep = "")]
    ...
    all.S = (c(1:ncol(da5)) %% 7) == 1
    all.E = (c(1:ncol(da5)) %% 7) == 2
    all.D = (c(1:ncol(da5)) %% 7) == 3
    all.U = (c(1:ncol(da5)) %% 7) == 4
    all.R = (c(1:ncol(da5)) %% 7) == 6
    ...
    S2.diff[i,j] = round(sum((da5[i,(all.S)][,-j])[,c(1:ncol(da5[i,(all.S)][,-j])-1)]))
    E2.diff[i,j] = round(sum((da5[i,(all.E)][,-j])[,c(1:ncol(da5[i,(all.E)][,-j])-1)]))
    D2.diff[i,j] = round(sum((da5[i,(all.D)][,-j])[,c(1:ncol(da5[i,(all.D)][,-j])-1)]))
    U2.diff[i,j] = round(sum((da5[i,(all.U)][,-j])[,c(1:ncol(da5[i,(all.U)][,-j])-1)]))
    R2.diff[i,j] = round(sum((da5[i,(all.R)][,-j])[,c(1:ncol(da5[i,(all.R)][,-j])-1)]))
    ...
    #-----------------------------------------------------------------------------------------------------------------#
    ########## 2 weeks on 2 weeks off : School day Monday to Friday in the weeks on 
    onoff2.week = c(seq(0,T,21)-6, seq(0,T,21)-5, seq(0,T,21)-4, seq(0,T,21)-3, seq(0,T,21)-2, 
                    seq(0,T,28)-6, seq(0,T,28)-5, seq(0,T,28)-4, seq(0,T,28)-3, seq(0,T,28)-2, 
                    seq(0,T,7)-1, seq(0,T,7))
    
    if (!is.na(match(i, setdiff(0:T,onoff2.week))) == TRUE){  #if time i is weekday
    }
    ...
    # Add the results of each time step i and each bubble j of each compartments to the dataframe da5
    da5[i,paste("S.B", j, sep = "")] <- S[i,j]
    da5[i,paste("E.B", j, sep = "")] <- E[i,j]
    da5[i,paste("D.B", j, sep = "")] <- D[i,j]
    da5[i,paste("U.B", j, sep = "")] <- U[i,j]
    da5[i,paste("Q.B", j, sep = "")] <- Q[i,j]
    da5[i,paste("R.B", j, sep = "")] <- R[i,j]
    
    da5[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,])  
    da5[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,])  
    da5[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,])  
    da5[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,])  
    da5[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,])  
    da5[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,]) 
    
    # Sum up the total dynamic of students for all bubbles
    da5$S.Student[i] <- sum(S[i,])
    da5$E.Student[i] <- sum(E[i,])
    da5$D.Student[i] <- sum(D[i,])
    da5$U.Student[i] <- sum(U[i,])
    da5$Q.Student[i] <- sum(Q[i,])
    da5$R.Student[i] <- sum(R[i,])
    da5$N.Student[i] <- 100
...
for (i in 1:T){
  ...
  da5$Rt.Student[i] <- (da5$E.Student[i+1] + da5$D.Student[i+1] + da5$U.Student[i+1] + da5$Q.Student[i+1]) / 
    (da5$E.Student[i] + da5$D.Student[i] + da5$U.Student[i] + da5$Q.Student[i])
  
  da5$Rt.Staff[i] <- (da5$E.Staff[i+1] + da5$D.Staff[i+1] + da5$U.Staff[i+1] + da5$Q.Staff[i+1]) / 
    (da5$E.Staff[i] + da5$D.Staff[i] + da5$U.Staff[i] + da5$Q.Staff[i])
}
...
# Students
X_lim <- seq(1,T,by=1)
plot(da5$S.Student~X_lim, pch=15, col="darkTurquoise", main = "Variation in Time, 2 weeks on 2 weeks off school - Students", type = "l", xlab = "Time", ylab = "Number of Students", xlim = c(0,T), ylim = c(0,100))

lines(da5$S.Student, col="DeepPink", lty=1)
lines(da5$E.Student, col="darkTurquoise", lty=1)
lines(da5$D.Student, col="RosyBrown", lty=1)
lines(da5$U.Student, col="green", lty=1)
lines(da5$Q.Student, col="darkgreen", lty=1)
lines(da5$R.Student, col="darkblue", lty=1)
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da5$S.Staff~X_lim, pch=15, col="darkTurquoise", main = "Variation in Time, 2 weeks on 2 weeks off school - Staffs", type = "l", xlab = "Time", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,35))

lines(da5$S.Staff, col="DeepPink", lty=1)
lines(da5$E.Staff, col="darkTurquoise", lty=1)
lines(da5$D.Staff, col="RosyBrown", lty=1)
lines(da5$U.Staff, col="green", lty=1)
lines(da5$Q.Staff, col="darkgreen", lty=1)
lines(da5$R.Staff, col="darkblue", lty=1)
