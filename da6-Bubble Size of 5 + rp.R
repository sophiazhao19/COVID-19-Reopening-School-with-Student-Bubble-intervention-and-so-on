# Variations in Bubble Size 5 Students in each bubble
# (da6)
###################################################################################################################
...
r.values = c(4,3,5,2,3)  # r:contact r people (should be changing with time), in the same order as p (RHO)

T = 366
N = 5 # numbers of in each bubble

###################################################################################################################

# Define dataframe - students bubbles 1-20 and total staff
# Define array 
B <- matrix(rep(0), T, 7)
A <- array(rep(B), dim = c(T, 7, 21))

A[,1,] <- 5 #  S
A[,7,] <- 5 #  N
A[,1,21] <- 55  # 30 staffs are S initially
A[,7,21] <- 55 #  55 staffs in total (none of them work from home)
A[1,,1] <- c(4,1,0,0,0,0,5)   # initials

dimnames(A) <- list(1:T, c("S","E","D","U","Q","R","N"), c(sprintf("B%d", 1:20),"Staff"))  # name the array
...
# Set initials
S <- matrix(rep(0), T+1, 20)
E <- matrix(rep(0), T+1, 20)
D <- matrix(rep(0), T+1, 20)
U <- matrix(rep(0), T+1, 20)
Q <- matrix(rep(0), T+1, 20)
R <- matrix(rep(0), T+1, 20)
r <- matrix(rep(0), T+1, 20)
...
for (i in 1:T){
  for (j in 1:20){  
    ...
    # initals
    S[1,j] =  da6[1,paste("S.B", j, sep = "")]
    E[1,j] =  da6[1,paste("E.B", j, sep = "")]
    D[1,j] =  da6[1,paste("D.B", j, sep = "")]
    U[1,j] =  da6[1,paste("U.B", j, sep = "")]
    Q[1,j] =  da6[1,paste("Q.B", j, sep = "")]
    R[1,j] =  da6[1,paste("R.B", j, sep = "")]
    
    # STUDENT to STUDENT in the SAME BUBBLE
    S1[1,j] =  da6[1,paste("S.B", j, sep = "")]
    E1[1,j] =  da6[1,paste("E.B", j, sep = "")]
    D1[1,j] =  da6[1,paste("D.B", j, sep = "")]
    U1[1,j] =  da6[1,paste("U.B", j, sep = "")]
    Q1[1,j] =  da6[1,paste("Q.B", j, sep = "")]
    R1[1,j] =  da6[1,paste("R.B", j, sep = "")]
    ...
    all.S = (c(1:ncol(da7)) %% 7) == 1
    all.E = (c(1:ncol(da7)) %% 7) == 2
    all.D = (c(1:ncol(da7)) %% 7) == 3
    all.U = (c(1:ncol(da7)) %% 7) == 4
    all.R = (c(1:ncol(da7)) %% 7) == 6
    ...
    S2.diff[i,j] = round(sum((da7[i,(all.S)][,-j])[,c(1:ncol(da7[i,(all.S)][,-j])-1)]))
    E2.diff[i,j] = round(sum((da7[i,(all.E)][,-j])[,c(1:ncol(da7[i,(all.E)][,-j])-1)]))
    D2.diff[i,j] = round(sum((da7[i,(all.D)][,-j])[,c(1:ncol(da7[i,(all.D)][,-j])-1)]))
    U2.diff[i,j] = round(sum((da7[i,(all.U)][,-j])[,c(1:ncol(da7[i,(all.U)][,-j])-1)]))
    R2.diff[i,j] = round(sum((da7[i,(all.R)][,-j])[,c(1:ncol(da7[i,(all.R)][,-j])-1)]))
    ...
    S.Std_Std[1,] = c(rep(0,20))  # S.Std_Std : total change of students in tutor group
    S.Std1[1,] = c(rep(0,20))
    
    
    # The change of S for tutor group j with students in both bubble j and 11-j
    if (j <= 5){ # 5 tutor groups in total
      S.Std_Std[i+1,j] = - (S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j])*p[1]*((D[i,j]+e*U[i,j])+(D[i,11-j]+e*U[i,11-j])+(D[i,10+j]+e*U[i,10+j])+(D[i,21-j]+e*U[i,21-j]))*1/24
      
      # As the change is less than 0.5, separate the new cases of infections according to the percentage of S in each bubble
      if (S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j] > 0){
        S.Std1[i+1,j] = S.Std_Std[i+1,j]*S[i,j]/(S[i,j] + S[i,10+j] + S[i,11-j] + S[i,21-j])  # S.Std1 : total change of students in  bubble j by having tutor group
        S.Std1[i+1,11-j] = S.Std_Std[i+1,j]*S[i,11-j]/(S[i,j] + S[i,10+j] + S[i,11-j] + S[i,21-j])
        S.Std1[i+1,10+j] = S.Std_Std[i+1,j]*S[i,10+j]/(S[i,j] + S[i,10+j] + S[i,11-j] + S[i,21-j])
        S.Std1[i+1,21-j] = S.Std_Std[i+1,j]*S[i,21-j]/(S[i,j] + S[i,10+j] + S[i,11-j] + S[i,21-j])
      }
    ...
    if (j <= 5){ # 5 tutor groups in total
      if (S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j] > 0){
        S.Std2[i+1,j] = - ((S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,j]/(S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j]))
        S.Std2[i+1,11-j] = - ((S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,11-j]/(S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j]))
        S.Std2[i+1,10+j] = - ((S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,10+j]/(S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j]))
        S.Std2[i+1,21-j] = - ((S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j])*p[3]*(D.Stf[i,j]+e*U.Stf[i,j])*1/24)*(S[i,21-j]/(S[i,j]+S[i,11-j]+S[i,10+j]+S[i,21-j]))
      }
    ...
    # initial - there is no change of S and E initially
    S.Stf[1,] = c(rep(1,5), rep(0,15)) 
    E.Stf[1,] = D.Stf[1,] = U.Stf[1,] = Q.Stf[1,] = R.Stf[1,] = rep(0,20)
    
    
    # The total number of S for staff in tutor group j 
    if (j <= 5){ # 5 tutor groups in total
      if (S.Stf[i,j] > 0){
        S.Stf[i+1,j] = S.Stf[i,j] - S.Stf[i,j]*p[4]*((D[i,j]+e*U[i,j])+(D[i,11-j]+e*U[i,11-j])+(D[i,10+j]+e*U[i,10+j])+(D[i,21-j]+e*U[i,21-j]))*1/24
      }
    ...
    # NO Staff working from home that can come and take replacement
    ...
    # Add the results of each time step i and each bubble j of each compartments to the dataframe da6
    da6[i,paste("S.B", j, sep = "")] <- S[i,j]
    da6[i,paste("E.B", j, sep = "")] <- E[i,j]
    da6[i,paste("D.B", j, sep = "")] <- D[i,j]
    da6[i,paste("U.B", j, sep = "")] <- U[i,j]
    da6[i,paste("Q.B", j, sep = "")] <- Q[i,j]
    da6[i,paste("R.B", j, sep = "")] <- R[i,j]
    
    da6[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,]) + sum(S.Stf[i,])
    da6[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,]) + sum(E.Stf[i,])
    da6[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,]) + sum(D.Stf[i,])
    da6[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,]) + sum(U.Stf[i,])
    da6[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,]) + sum(Q.Stf[i,])
    da6[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,]) + sum(R.Stf[i,])
    
    # Sum up the total dynamic of students for all bubbles
    da6$S.Student[i] <- sum(S[i,])
    da6$E.Student[i] <- sum(E[i,])
    da6$D.Student[i] <- sum(D[i,])
    da6$U.Student[i] <- sum(U[i,])
    da6$Q.Student[i] <- sum(Q[i,])
    da6$R.Student[i] <- sum(R[i,])
    da6$N.Student[i] <- 100
...
for (i in 1:T){
  ...
  da6$Staff.rp[i] <- sum(Staff.rp[i,])  
  
  # reproduction rate Rt
  da6$Rt.Student[i] <- (da6$E.Student[i+1] + da6$D.Student[i+1] + da6$U.Student[i+1] + da6$Q.Student[i+1]) / 
    (da6$E.Student[i] + da6$D.Student[i] + da6$U.Student[i] + da6$Q.Student[i])
  
  da6$Rt.Staff[i] <- (da6$E.Staff[i+1] + da6$D.Staff[i+1] + da6$U.Staff[i+1] + da6$Q.Staff[i+1]) / 
    (da6$E.Staff[i] + da6$D.Staff[i] + da6$U.Staff[i] + da6$Q.Staff[i])
}
...
# Students
X_lim <- seq(1,T,by=1)
plot(da6$S.Student~X_lim, pch=15, col="black", main = "SEDUQR Model for Students", type = "l", xlab = "Time (day)", ylab = "Number of Students", xlim = c(0,T), ylim = c(0,100))

lines(da6$S.Student, col="DeepPink", lty=1)
lines(da6$E.Student, col="darkTurquoise", lty=1)
lines(da6$D.Student, col="RosyBrown", lty=1)
lines(da6$U.Student, col="green", lty=1)
lines(da6$Q.Student, col="darkgreen", lty=1)
lines(da6$R.Student, col="darkblue", lty=1)
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da6$S.Staff~X_lim, pch=15, col="black", main = "SEDUQR Model for Staffs", type = "l", xlab = "Time (day)", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,55))

lines(da6$S.Staff, col="DeepPink", lty=1)
lines(da6$E.Staff, col="darkTurquoise", lty=1)
lines(da6$D.Staff, col="RosyBrown", lty=1)
lines(da6$U.Staff, col="green", lty=1)
lines(da6$Q.Staff, col="darkgreen", lty=1)
lines(da6$R.Staff, col="darkblue", lty=1)

legend(300, 55,c("S","E","D", "U", "Q", "R"),col=c("DeepPink","darkTurquoise","RosyBrown", "green", "darkgreen", "darkblue"),
       text.col=c("DeepPink","darkTurquoise","RosyBrown", "green", "darkgreen", "darkblue"),lty=c(1,1,1,1,1,1),
       cex=0.5)
