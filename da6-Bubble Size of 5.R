...
for (i in 1:T){
  for (j in 1:20){  
    ...
    da6[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,]) 
    da6[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,]) 
    da6[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,]) 
    da6[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,])
    da6[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,]) 
    da6[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,]) 
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da6$S.Staff~X_lim, pch=15, col="black", main = "SEDUQR Model for Staffs", type = "l", xlab = "Time (day)", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,35))
