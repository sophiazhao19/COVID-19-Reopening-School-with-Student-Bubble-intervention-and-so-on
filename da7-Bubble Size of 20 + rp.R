...
T = 366
N = 20 # student number in each bubble


###################################################################################################################

# Define dataframe - students bubbles 1-5 and total staff
# Define array 
B <- matrix(rep(0), T, 7)
A <- array(rep(B), dim = c(T, 7, 6))

A[,1,] <- 20 #  S
A[,7,] <- 20 #  N
A[,1,6] <- 15  # 30 staffs are S initially
A[,7,6] <- 55 #  55 staffs in total (20 of them work from home)
A[1,,1] <- c(19,1,0,0,0,0,20)   # initials

dimnames(A) <- list(1:T, c("S","E","D","U","Q","R","N"), c(sprintf("B%d", 1:5),"Staff"))  # name the array
...
# Set initials
S <- matrix(rep(0), T+1, 5)
E <- matrix(rep(0), T+1, 5)
D <- matrix(rep(0), T+1, 5)
U <- matrix(rep(0), T+1, 5)
Q <- matrix(rep(0), T+1, 5)
R <- matrix(rep(0), T+1, 5)
r <- matrix(rep(0), T+1, 5)
...
for (i in 1:T){
  for (j in 1:5){  
    ...
    # initals
    S[1,j] =  da7[1,paste("S.B", j, sep = "")]
    E[1,j] =  da7[1,paste("E.B", j, sep = "")]
    D[1,j] =  da7[1,paste("D.B", j, sep = "")]
    U[1,j] =  da7[1,paste("U.B", j, sep = "")]
    Q[1,j] =  da7[1,paste("Q.B", j, sep = "")]
    R[1,j] =  da7[1,paste("R.B", j, sep = "")]
    
    # STUDENT to STUDENT in the SAME BUBBLE
    S1[1,j] =  da7[1,paste("S.B", j, sep = "")]
    E1[1,j] =  da7[1,paste("E.B", j, sep = "")]
    D1[1,j] =  da7[1,paste("D.B", j, sep = "")]
    U1[1,j] =  da7[1,paste("U.B", j, sep = "")]
    Q1[1,j] =  da7[1,paste("Q.B", j, sep = "")]
    R1[1,j] =  da7[1,paste("R.B", j, sep = "")]
    ...
    Staff.rp[1,j] = 8  # the staff can come to teach in the next day that are working from home
    ...
    # Add the results of each time step i and each bubble j of each compartments to the dataframe da7
    da7[i,paste("S.B", j, sep = "")] <- S[i,j]
    da7[i,paste("E.B", j, sep = "")] <- E[i,j]
    da7[i,paste("D.B", j, sep = "")] <- D[i,j]
    da7[i,paste("U.B", j, sep = "")] <- U[i,j]
    da7[i,paste("Q.B", j, sep = "")] <- Q[i,j]
    da7[i,paste("R.B", j, sep = "")] <- R[i,j]
    
    da7[i,paste("S.", "Staff", sep = "")] <- sum(S.Staff[i,]) 
    da7[i,paste("E.", "Staff", sep = "")] <- sum(E.Staff[i,]) 
    da7[i,paste("D.", "Staff", sep = "")] <- sum(D.Staff[i,]) 
    da7[i,paste("U.", "Staff", sep = "")] <- sum(U.Staff[i,])
    da7[i,paste("Q.", "Staff", sep = "")] <- sum(Q.Staff[i,]) 
    da7[i,paste("R.", "Staff", sep = "")] <- sum(R.Staff[i,]) 
    
    # Sum up the total dynamic of students for all bubbles
    da7$S.Student[i] <- sum(S[i,])
    da7$E.Student[i] <- sum(E[i,])
    da7$D.Student[i] <- sum(D[i,])
    da7$U.Student[i] <- sum(U[i,])
    da7$Q.Student[i] <- sum(Q[i,])
    da7$R.Student[i] <- sum(R[i,])
    da7$N.Student[i] <- 100
...
for (i in 1:T){
  ...
  da7$Staff.rp[i] <- sum(Staff.rp[i,])  
  
  # reproduction rate Rt
  da7$Rt.Student[i] <- (da7$E.Student[i+1] + da7$D.Student[i+1] + da7$U.Student[i+1] + da7$Q.Student[i+1]) / 
    (da7$E.Student[i] + da7$D.Student[i] + da7$U.Student[i] + da7$Q.Student[i])
  
  da7$Rt.Staff[i] <- (da7$E.Staff[i+1] + da7$D.Staff[i+1] + da7$U.Staff[i+1] + da7$Q.Staff[i+1]) / 
    (da7$E.Staff[i] + da7$D.Staff[i] + da7$U.Staff[i] + da7$Q.Staff[i])
}
...
# Students
X_lim <- seq(1,T,by=1)
plot(da7$S.Student~X_lim, pch=15, col="black", main = "SEDUQR Model for Students", type = "l", xlab = "Time (day)", ylab = "Number of Students", xlim = c(0,T), ylim = c(0,100))

lines(da7$S.Student, col="DeepPink", lty=1)
lines(da7$E.Student, col="darkTurquoise", lty=1)
lines(da7$D.Student, col="RosyBrown", lty=1)
lines(da7$U.Student, col="green", lty=1)
lines(da7$Q.Student, col="darkgreen", lty=1)
lines(da7$R.Student, col="darkblue", lty=1)
...
# Staffs
X_lim <- seq(1,T,by=1)
plot(da7$S.Staff~X_lim, pch=15, col="black", main = "SEDUQR Model for Staffs", type = "l", xlab = "Time (day)", ylab = "Number of Staffs", xlim = c(0,T), ylim = c(0,35))

lines(da7$S.Staff, col="DeepPink", lty=1)
lines(da7$E.Staff, col="darkTurquoise", lty=1)
lines(da7$D.Staff, col="RosyBrown", lty=1)
lines(da7$U.Staff, col="green", lty=1)
lines(da7$Q.Staff, col="darkgreen", lty=1)
lines(da7$R.Staff, col="darkblue", lty=1)
